...
export const PH = {
  ...
  // 使用者在甘特圖手動新增的任務(不在上面固定流程裡),統一歸這個相位、排在最後。
  custom: { n: "自訂任務",   c: "#888",    tone: "lavender" },
};

// 相位顯示順序(甘特圖依此排相位區塊)。不能用 Object.keys(PH):JS 會把「像整數」的鍵(2,3,5…)
// 重排到字母鍵(1A,1B,4A)前面,得到 2,3,5,6,7,8,9,1A,1B,4A —— 順序是錯的。這裡明列正確順序;
// 自訂任務排在最前面(使用者手動加的任務優先顯示),其後才是開案→募資後續的固定流程。
export const PHASE_ORDER = ["custom", "1A", "1B", "2", "3", "4A", "5", "6", "7", "8", "9"];
